...
function renderField(key, val, path, depth, maxDepth) {
    ...
    return `
      <div class="ed-group">
        <div class="ed-group-head">
          <span class="ed-group-icon">▾</span>
          <span class="ed-label">${label}</span>
          <span class="ed-badge">${Object.keys(val).length}</span>
        </div>
        <div class="ed-group-body">
          ${renderFields(val, path, depth + 1, maxDepth)}
        </div>
      </div>`;
  }
  return `
...
function renderFields(obj, basePath, depth, maxDepth) {
  return Object.keys(obj).filter((k) => typeof obj[k] !== "function").map((k) => renderField(k, obj[k], `${basePath}.${k}`, depth, maxDepth)).join("");
}
...
function Editor(object, name = "Unknown Object ✏️", options = {}) {
  ...
  wrapper.addEventListener("change", (e) => {
    ...
    if (!path)
      return;
    if (el.type === "checkbox")
      mutations[path] = el.checked;
    else if (el.type === "number")
      mutations[path] = Number(el.value);
    else
      mutations[path] = el.value;
  });
  wrapper.querySelectorAll(".ed-group-head").forEach((head) => {
    head.addEventListener("click", () => {
      head.closest(".ed-group").classList.toggle("collapsed");
    });
  });
  const actionOutput = wrapper.querySelector("#ed-action-output");
  wrapper.querySelectorAll(".ed-action-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const fnKey = btn.dataset.fn;
